...
exports.new_withdrawal_transaction = async (req, res) => {
  try {
    ...
    const bank = await Bank.findOne({ where: { user_id: userId } });
    ...
    if (!bank) {
      res
        .status(200)
        .json({ success: false, message: "Bạn chưa thêm tài khoản ngân hàng" });
      return;
    }
    ...
    const transaction = await WithdrawalTransaction.create({
      ...
      balanceNow, // Cập nhật giá trị balanceNow trong bảng WithdrawalTransaction
    });
    res.status(200).json({
      success: true,
